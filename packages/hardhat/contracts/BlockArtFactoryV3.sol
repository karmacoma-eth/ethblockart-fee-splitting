...
import "./BArt.sol";
import "./BStyle.sol";
import "./IBlockArtFactory.sol";
import "./BlockArtVault.sol";

contract BlockArtFactoryV3 is Ownable {
    ...
    /// @dev Mint BlockArt NFTs and deposit funds to BlockArtVault
    ...
    /// @param styleFeeBasisPoints The style fee in basis points (1/100th of a percent), selected by the minter
    /// @param charityFeeBasisPoints The charity fee in basis points (1/100th of a percent), selected by the minter
    function mintArt(
        address to,
        uint256 blockNumber,
        uint256 styleId,
    ...
    function burnArt(uint256 tokenId) external payable onlyArtOwner(tokenId) {
        ...
        BlockArtVault(vaultAddr).depositToTreasury{value: msg.value}();
        ...
        returns (uint256)
    {
        ...
        if (_psfb > 0) {
            ...
            if (blockNumber >= block.number) {
                price += priceCeil;
            } else if (blockNumber + dutchLength >= block.number) {
                // only if blocknumber in dutch range from current block
                uint256 blockDiff = block.number - blockNumber;
                uint256 priceDiff = priceCeil - (blockDiff * priceCeil / dutchLength);
                price += priceDiff;
            }
        }

        uint256 sfa = price * _style.getStyleFeeMul(styleId) / 100 - price; //style fee amount
    ...
    function setVaultAddr(address _vaultAddr) external onlyOwner {
        vaultAddr = _vaultAddr;
    }
    ...
    function transferOldFaOwnership(address to) external onlyOwner {
        Ownable _oldFa = Ownable(oldFactoryAddr);
